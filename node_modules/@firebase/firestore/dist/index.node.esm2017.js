 ...
 * limitations under the License.
 */
const LOG_TAG$9 = 'LruGarbageCollector';
    ...
    scheduleGC(localStore) {
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, async () => {
            ...
            try {
                ...
                if (e.name === 'IndexedDbTransactionError') {
                    logDebug(LOG_TAG$9, 'Ignoring IndexedDB error during garbage collection: ', e);
                }
                else {
...
class SimpleDb {
    ...
    static openOrCreate(name, version, schemaConverter) {
        ...
        logDebug(LOG_TAG$a, 'Opening database:', name);
        return new PersistencePromise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                logDebug(LOG_TAG$a, 'Database "' + name + '" requires upgrade from version:', event.oldVersion);
                const db = event.target.result;
                schemaConverter
                    .createOrUpgrade(db, request.transaction, event.oldVersion, SCHEMA_VERSION)
                    .next(() => {
                    logDebug(LOG_TAG$a, 'Database upgrade to version ' + SCHEMA_VERSION + ' complete');
                });
            };
        }).toPromise();
    }
    ...
    static delete(name) {
        logDebug(LOG_TAG$a, 'Removing database:', name);
    ...
    async runTransaction(mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$a, 'Transaction failed with error: %s. Retrying: %s.', error.message, retryable);
                if (!retryable) {
...
class SimpleDbTransaction {
    ...
    abort(error) {
        ...
        if (!this.aborted) {
            logDebug(LOG_TAG$a, 'Aborting transaction:', error ? error.message : 'Client-initiated abort');
            this.aborted = true;
...
class SimpleDbStore {
    ...
    put(keyOrValue, value) {
        let request;
        if (value !== undefined) {
            logDebug(LOG_TAG$a, 'PUT', this.store.name, keyOrValue, value);
            request = this.store.put(value, keyOrValue);
        }
        else {
            logDebug(LOG_TAG$a, 'PUT', this.store.name, '<auto-key>', keyOrValue);
     ...
     */
    add(value) {
        logDebug(LOG_TAG$a, 'ADD', this.store.name, value, value);
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$a, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
        logDebug(LOG_TAG$a, 'DELETE', this.store.name, key);
     ...
     */
    count() {
        logDebug(LOG_TAG$a, 'COUNT', this.store.name);
    ...
    deleteAll(indexOrRange, range) {
        logDebug(LOG_TAG$a, 'DELETE ALL', this.store.name);
 ...
 * limitations under the License.
 */
const LOG_TAG$b = 'IndexedDbPersistence';
/**
...
class IndexedDbPersistence {
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.simpleDb
            ...
            logDebug(LOG_TAG$b, 'Releasing owner lease after error during lease refresh', e);
    ...
    canActAsPrimary(txn) {
        ...
        return store
            ...
            .next(canActAsPrimary => {
            if (this.isPrimary !== canActAsPrimary) {
                logDebug(LOG_TAG$b, `Client ${canActAsPrimary ? 'is' : 'is not'} eligible for a primary lease.`);
            }
    ...
    releasePrimaryLeaseIfHeld(txn) {
        ...
        return store.get(DbPrimaryClient.key).next(primaryClient => {
            if (this.isLocalClient(primaryClient)) {
                logDebug(LOG_TAG$b, 'Releasing primary lease.');
    ...
    isClientZombied(clientId) {
        try {
            const isZombied = this.webStorage.getItem(this.zombiedClientLocalStorageKey(clientId)) !==
                null;
            logDebug(LOG_TAG$b, `Client '${clientId}' ${isZombied ? 'is' : 'is not'} zombied in LocalStorage`);
            return isZombied;
        }
        catch (e) {
            // Gracefully handle if LocalStorage isn't working.
            logError(LOG_TAG$b, 'Failed to get zombied client id.', e);
            return false;
        }
    }
    /**
        ...
        this.retryableTail = this.retryableTail.then(() => {
            ...
            const retryingOp = async () => {
                try {
                    ...
                    if (e.name === 'IndexedDbTransactionError') {
                        logDebug(LOG_TAG$e, 'Operation failed with retryable error: ' + e);
...
class ArrayUnionFieldValueImpl extends FieldValueImpl {
    ...
    toFieldTransform(context) {
        ...
        const parsedElements = this._elements.map((element, i) => parseData(element, parseContext.childContextForArray(i)));
...
class GrpcConnection {
    ...
    invokeRPC(rpcName, request, token) {
        ...
        return nodePromise((callback) => {
            logDebug(LOG_TAG$f, `RPC '${rpcName}' invoked with request:`, request);
            return stub[rpcName](request, metadata, (grpcError, value) => {
                if (grpcError) {
                    logDebug(LOG_TAG$f, `RPC '${rpcName}' failed with error:`, grpcError);
                    callback(new FirestoreError(mapCodeFromRpcCode(grpcError.code), grpcError.message));
                }
                else {
                    logDebug(LOG_TAG$f, `RPC '${rpcName}' completed with response:`, value);
    ...
    invokeStreamingRPC(rpcName, request, token) {
        ...
        logDebug(LOG_TAG$f, `RPC '${rpcName}' invoked (streaming) with request:`, request);
        const stub = this.ensureActiveStub();
        const metadata = createMetadata(this.databaseInfo, token);
        const stream = stub[rpcName](request, metadata);
        stream.on('data', (response) => {
            logDebug(LOG_TAG$f, `RPC ${rpcName} received result:`, response);
            results.push(response);
        });
        stream.on('end', () => {
            logDebug(LOG_TAG$f, `RPC '${rpcName}' completed.`);
    ...
    openStream(rpcName, token) {
        ...
        const stream = new StreamBridge({
            sendFn: (msg) => {
                if (!closed) {
                    logDebug(LOG_TAG$f, 'GRPC stream sending:', msg);
                    try {
                    ...
                    logDebug(LOG_TAG$f, 'Not sending because gRPC stream is closed:', msg);
                }
            },
            closeFn: () => {
                logDebug(LOG_TAG$f, 'GRPC stream closed locally via close().');
                close();
            }
        });
        grpcStream.on('data', (msg) => {
            if (!closed) {
                logDebug(LOG_TAG$f, 'GRPC stream received:', msg);
                stream.callOnMessage(msg);
            }
        });
        grpcStream.on('end', () => {
            logDebug(LOG_TAG$f, 'GRPC stream ended.');
            close();
        });
        grpcStream.on('error', (grpcError) => {
            logDebug(LOG_TAG$f, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
            const code = mapCodeFromRpcCode(grpcError.code);
            close(new FirestoreError(code, grpcError.message));
        });
        logDebug(LOG_TAG$f, 'Opening GRPC stream');
